...
//        ProductPriceHistory priceOne = ProductPriceHistory.of(SnowflakeUtil.nextId(), 15000, false, productOne);
//        ProductPriceHistory priceTwo = ProductPriceHistory.of(SnowflakeUtil.nextId(), 25000, true, productOne);
//        ProductPriceHistory priceThree = ProductPriceHistory.of(SnowflakeUtil.nextId(), 8000, true, productTwo);
//        List<ProductPriceHistory> productPriceHistoryList = Arrays.asList(priceOne, priceTwo, priceThree);
...
//        productPriceRepository.saveAll(productPriceHistoryList);
    ...
    public void enterWrongLanguage() {
        ...
        Product product = productRepository.findByProductId(productId).orElseThrow(() -> new BusinessException(ExceptionCode.PRODUCT_NOT_FOUND));
        ProductInfo productInfo = productRepository.getProductInfoByProductId(productId, language).orElseThrow(() -> new BusinessException(ExceptionCode.PRODUCT_INFO_NOT_FOUND));
//        ProductPriceHistory price = productPriceRepository.findCurrentPriceByProduct(productId).orElseThrow(() -> new BusinessException(ExceptionCode.PRODUCT_PRICE_NOT_FOUND));
